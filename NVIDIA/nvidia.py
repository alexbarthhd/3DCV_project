...
It's build up with 9 layers, beginning with a normalization layer, followed by 5
convolution layers, and 3 full connected layers.

The normaliation layer allows a acceleration in processing via GPU.
The convolution properties are chosen empirically through a series of experiments
and designed to perform feature extraction. This follows at the end with three
fully connected layers. They are designed to function as a controller for steering.
...
class NvidiaModel(KerasPilot):
    def __init__(self, num_outputs=2, input_shape=(120, 160, 3), roi_crop=(0, 0), *args, **kwargs):
        ...
        self.model = Nvidia(num_outputs, input_shape, roi_crop)
        self.compile()
...
class NvidiaImprovedModel(KerasPilot):
    def __init__(self, num_outputs=2, input_shape=(120, 160, 3), roi_crop=(0, 0), *args, **kwargs):
        super(NvidiaImprovedModel, self).__init__(*args, **kwargs)
        self.model = NvidiaImproved(num_outputs, input_shape, roi_crop)
        self.compile()
...
def NvidiaImproved(num_outputs, input_shape, roi_crop):
